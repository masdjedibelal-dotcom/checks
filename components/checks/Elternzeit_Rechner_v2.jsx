...
function berechne(p){
  const{netto1,netto2,monate1,monate2,modell}=p;
  const eg1=Math.min(Math.max(netto1*0.65,300),1800);
  const eg2=Math.min(Math.max(netto2*0.65,300),1800);
  const hausNormal=netto1+netto2;
  let eg=0,hausElternzeit=0;
  if(modell==="abwechselnd"){eg=eg1+eg2;hausElternzeit=eg;}
  else if(modell==="gleichzeitig"){eg=eg1+eg2;hausElternzeit=eg;}
  else{eg=monate1>0?eg1:0;eg+=monate2>0?eg2:0;hausElternzeit=eg;}
  const luecke=Math.max(0,hausNormal-hausElternzeit);
  const gesamt14=Math.min(14,monate1+monate2);
  return{eg1,eg2,hausNormal,hausElternzeit,luecke,gesamt14};
}
...
export default function ElternzeitRechner(){
  ...
  const[p,setP]=useState({netto1:2800,netto2:2200,monate1:12,monate2:2,modell:"abwechselnd"});
  const set=(k,v)=>setP(x=>({...x,[k]:v}));const goTo=(ph)=>{setAk(k=>k+1);setPhase(ph);window.scrollTo({top:0});};
  const R=berechne(p);
  const CHECKLISTE=[
    {l:"Risikolebensversicherung",sub:"Versicherungssumme auf neue Familiensituation anpassen",dringend:true},
    {l:"Privathaftpflicht",sub:"Kind aufnehmen — häufig kostenlos als Familienerweiterung",dringend:true},
    {l:"Berufsunfähigkeit",sub:"BU-Rente auf Elternzeit-Einkommen prüfen",dringend:false},
    {l:"Krankenversicherung",sub:"Kind in GKV-Familienversicherung oder eigene PKV einschreiben",dringend:true},
    {l:"Riester-Rente",sub:"Kinderzulage (185 € / 300 €) beantragen — läuft nicht automatisch!",dringend:true},
    {l:"Hausrat",sub:"Neue Wohnfläche und Gegenstände melden",dringend:false},
  ];
  if(danke)return(
    <div style={{...T.page,"--accent":C}}><div style={T.header}><div style={T.logo}><div style={T.logoMk}><LogoSVG/></div><span style={{fontSize:"13px",fontWeight:"600",color:"#111"}}>{MAKLER.firma}</span></div><span style={T.badge}>Elternzeit</span></div>
    <div style={{padding:"48px 24px",textAlign:"center"}} className="fade-in">
      ...
      <div style={{fontSize:"14px",color:"#666",lineHeight:1.65,marginBottom:"32px"}}>Wir melden uns innerhalb von 24 Stunden und gehen alle Punkte mit Ihnen durch.</div>
  ...
  if(phase===2){
    ...
    return(
      <div style={{...T.page,"--accent":C}} key={ak} className="fade-in">
        <div style={T.header}><div style={T.logo}><div style={T.logoMk}><LogoSVG/></div><span style={{fontSize:"13px",fontWeight:"600",color:"#111"}}>{MAKLER.firma}</span></div><span style={T.badge}>Elternzeit</span></div>
        <div style={T.prog}><div style={T.progFil(100)}/></div>
        <div style={T.hero}><div style={T.eyebrow}>Ihre Elternzeit-Analyse</div><div style={T.h1}>{R.luecke>0?`${fmt(R.luecke)}/Monat Haushaltslücke`:"Gut geplant"}</div><div style={T.body}>Modell: {p.modell==="abwechselnd"?"Abwechselnd":p.modell==="gleichzeitig"?"Gleichzeitig":"Individuell"} · {p.monate1 + p.monate2} Monate gesamt</div></div>
        <div style={T.section}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"12px"}}>
            <div style={{border:"1px solid #e8e8e8",borderRadius:"10px",padding:"14px"}}><div style={{fontSize:"11px",fontWeight:"600",color:"#999",letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:"6px"}}>Haushalt heute</div><div style={{fontSize:"22px",fontWeight:"700",color:"#111",letterSpacing:"-0.5px"}}>{fmt(R.hausNormal)}</div><div style={{fontSize:"11px",color:"#aaa",marginTop:"2px"}}>pro Monat</div></div>
            <div style={{border:`1px solid ${R.luecke>0?WARN:"#e8e8e8"}`,borderRadius:"10px",padding:"14px"}}><div style={{fontSize:"11px",fontWeight:"600",color:R.luecke>0?WARN:"#999",letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:"6px"}}>Während Elternzeit</div><div style={{fontSize:"22px",fontWeight:"700",color:R.luecke>0?WARN:"#111",letterSpacing:"-0.5px"}}>{fmt(R.hausElternzeit)}</div><div style={{fontSize:"11px",color:"#aaa",marginTop:"2px"}}>pro Monat (Elterngeld)</div></div>
          </div>
          <div style={T.card}>
            {[
              {l:"Elterngeld Partner 1",v:fmt(R.eg1)+"/Mon.",sub:`65% von ${fmt(p.netto1)} Netto (max. 1.800 €)`},
              {l:"Elterngeld Partner 2",v:fmt(R.eg2)+"/Mon.",sub:`65% von ${fmt(p.netto2)} Netto (max. 1.800 €)`},
              {l:"Haushaltslücke",v:fmt(R.luecke)+"/Mon.",hl:R.luecke>0},
              {l:"Gesamte Elternzeit",v:`${p.monate1+p.monate2} Monate`,sub:`Davon max. ${R.gesamt14} Monate mit Elterngeld`},
        ...
        <div style={T.divider}/>
        <div style={T.section}>
          <div style={{fontSize:"11px",fontWeight:"600",color:"#999",letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:"12px"}}>Versicherungs-Checkliste</div>
          <div style={T.card}>
            {CHECKLISTE.map(({l,sub,dringend},i,arr)=>(
              <div key={i} style={{padding:"12px 16px",borderBottom:i<arr.length-1?"1px solid #f0f0f0":"none",display:"flex",alignItems:"flex-start",gap:"10px"}}>
                <div style={{width:"6px",height:"6px",borderRadius:"50%",background:dringend?WARN:C,marginTop:"5px",flexShrink:0}}/>
                <div><div style={{fontSize:"13px",fontWeight:"500",color:"#111"}}>{l}</div><div style={{fontSize:"11px",color:"#aaa",marginTop:"2px"}}>{sub}</div></div>
              </div>
            ))}
          </div>
          <div style={{fontSize:"11px",color:"#aaa",marginTop:"8px",display:"flex",gap:"12px"}}><span style={{display:"flex",alignItems:"center",gap:"4px"}}><div style={{width:"6px",height:"6px",borderRadius:"50%",background:WARN}}/> Dringend</span><span style={{display:"flex",alignItems:"center",gap:"4px"}}><div style={{width:"6px",height:"6px",borderRadius:"50%",background:C}}/> Empfohlen</span></div>
        </div>
        {demoCtx.isDemoMode ? (
  ...
  return(
    <div style={{...T.page,"--accent":C}} key={ak} className="fade-in">
      ...
      <div style={T.hero}><div style={T.eyebrow}>Elternzeit-Rechner</div><div style={T.h1}>Was bleibt in der Elternzeit übrig?</div><div style={T.body}>Elterngeld beider Partner, monatliche Haushaltslücke und Versicherungs-Checkliste.</div></div>
      <div style={T.section}>
        <div style={T.card}>
          <div style={T.row}><SliderField label="Nettoeinkommen Partner 1" value={p.netto1} min={1000} max={7000} step={100} unit="€/Mon" onChange={v=>set("netto1",v)}/></div>
          <div style={T.row}><SliderField label="Nettoeinkommen Partner 2" value={p.netto2} min={0} max={7000} step={100} unit="€/Mon" onChange={v=>set("netto2",v)}/></div>
          <div style={T.row}><SliderField label="Elternzeit Partner 1" value={p.monate1} min={0} max={14} step={1} unit="Monate" onChange={v=>set("monate1",v)}/></div>
          <div style={T.row}><SliderField label="Elternzeit Partner 2" value={p.monate2} min={0} max={14} step={1} unit="Monate" hint="Mindestens 2 Monate für Partner-Bonus (14 Monate gesamt)" onChange={v=>set("monate2",v)}/></div>
          <div style={T.rowLast}>
            <label style={T.fldLbl}>Modell</label>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px",marginTop:"8px"}}>
              {[["abwechselnd","Abwechselnd"],["gleichzeitig","Gleichzeitig"],["individuell","Individuell"]].map(([v,l])=><button key={v} style={T.optBtn(p.modell===v)} onClick={()=>set("modell",v)}>{l}</button>)}
            </div>
          </div>
        </div>
      </div>
      <div style={T.footer}><button style={T.btnPrim(false)} onClick={()=>goTo(2)}>Elternzeit berechnen</button></div>
    </div>
  );
}
